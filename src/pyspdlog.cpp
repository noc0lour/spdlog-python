...
public:
  ...
  virtual ~Sink() {}
  virtual void log(const spd::details::log_msg &msg) { _sink->log(msg); }
  ...
  spd::level::level_enum level() const { return _sink->level(); }
...
public:
  ...
  int level() const { return (int)_logger->level(); }
  ...
  spd::log_err_handler error_handler() { return _logger->error_handler(); }

protected:
  friend void set_default_logger(Logger *default_logger);
...
public:
  ConsoleLogger(const std::string &logger_name, bool multithreaded, bool stdout,
                bool colored)
      : Logger(logger_name) {
    if (stdout) {
      if (multithreaded) {
...
public:
  FileLogger(const std::string &logger_name, const std::string &filename,
             bool multithreaded, bool truncate = false)
      : Logger(logger_name) {
    if (multithreaded) {
...
public:
  RotatingLogger(const std::string &logger_name, const std::string &filename,
                 bool multithreaded, size_t max_file_size, size_t max_files)
      : Logger(logger_name) {
    if (multithreaded) {
      _logger = spd::rotating_logger_mt(logger_name, filename, max_file_size,
                                        max_files);
    } else {
      _logger = spd::rotating_logger_st(logger_name, filename, max_file_size,
                                        max_files);
    }
  }
};
...
public:
  DailyLogger(const std::string &logger_name, const std::string &filename,
              bool multithreaded = false, int hour = 0, int minute = 0)
      : Logger(logger_name) {
    if (multithreaded) {
...
public:
  SyslogLogger(const std::string &logger_name, const std::string &ident = "",
               bool multithreaded = false, int syslog_option = 0,
               int syslog_facilty = (1 << 3))
      : Logger(logger_name) {
    if (multithreaded) {
      _logger = spd::syslog_logger_mt(logger_name, ident, syslog_option,
                                      syslog_facilty);
    } else {
      _logger = spd::syslog_logger_st(logger_name, ident, syslog_option,
                                      syslog_facilty);
    }
  }
};
...
public:
  SinkLogger(const std::string &logger_name, const Sink &sink)
    ...
    _logger = std::shared_ptr<spd::logger>(
        new spd::logger(logger_name, sink.get_sink()));
  }
...
Logger get(const std::string &name) {
  ...
  if (logger)
    ...
    throw std::runtime_error(
        std::string("Logger name: " + name + " could name be found"));
}
...
public:
  ...
  const static int overrun_oldest{
      (int)spd::async_overflow_policy::overrun_oldest};
};
...
PYBIND11_MODULE(spdlog, m) {
  ...
  py::class_<LogLevel>(m, "LogLevel")
      .def_property_readonly_static("TRACE",
                                    [](py::object) { return LogLevel::trace; })
      .def_property_readonly_static("DEBUG",
                                    [](py::object) { return LogLevel::debug; })
      .def_property_readonly_static("INFO",
                                    [](py::object) { return LogLevel::info; })
      .def_property_readonly_static("WARN",
                                    [](py::object) { return LogLevel::warn; })
      .def_property_readonly_static("ERR",
                                    [](py::object) { return LogLevel::err; })
      .def_property_readonly_static(
          "CRITICAL", [](py::object) { return LogLevel::critical; })
      .def_property_readonly_static("OFF",
                                    [](py::object) { return LogLevel::off; });

  py::class_<AsyncOverflowPolicy>(m, "AsyncOverflowPolicy")
      .def_property_readonly_static(
          "block", [](py::object) { return AsyncOverflowPolicy::block; })
      .def_property_readonly_static("overrun_oldest", [](py::object) {
        return AsyncOverflowPolicy::overrun_oldest;
      });
  ...
  py::class_<Logger>(m, "Logger")
      ...
      .def("set_pattern", &Logger::set_pattern, py::arg("pattern"),
           py::arg("type") = spd::pattern_time_type::local)
  ...
  py::class_<ConsoleLogger, Logger>(m, "ConsoleLogger")
      .def(py::init<std::string, bool, bool, bool>(), py::arg("name"),
           py::arg("multithreaded") = false, py::arg("stdout") = true,
           py::arg("colored") = true);

  py::class_<FileLogger, Logger>(m, "FileLogger")
      .def(py::init<std::string, std::string, bool, bool>(), py::arg("name"),
           py::arg("filename"), py::arg("multithreaded") = false,
           py::arg("truncate") = false);
  py::class_<RotatingLogger, Logger>(m, "RotatingLogger")
      .def(py::init<std::string, std::string, bool, bool, bool>(),
           py::arg("name"), py::arg("filename"), py::arg("multithreaded"),
           py::arg("max_file_size"), py::arg("max_files"));
  py::class_<DailyLogger, Logger>(m, "DailyLogger")
      .def(py::init<std::string, std::string, bool, int, int>(),
           py::arg("name"), py::arg("filename"),
           py::arg("multithreaded") = false, py::arg("hour") = 0,
           py::arg("minute") = 0);
// SyslogLogger(const std::string& logger_name, const std::string& ident = "",
// int syslog_option = 0, int syslog_facilty = (1<<3))
#ifdef SPDLOG_ENABLE_SYSLOG
  py::class_<SyslogLogger, Logger>(m, "SyslogLogger")
      .def(py::init<std::string, std::string, int, int>(), py::arg("name"),
           py::arg("ident") = "", py::arg("syslog_option") = 0,
